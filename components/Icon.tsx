...
export function Icon({ name, ...props }: IconProps) {
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" {...props}>
      ...
      {name === "settings" && (
        <>
          <path d="M20 7h-9M14 17H5" {...shared} />
          <circle cx="17" cy="17" r="3" {...shared} />
          <circle cx="7" cy="7" r="3" {...shared} />
        </>
      )}
      {name === "panel" && (
        <>
          <rect x="3.5" y="4.5" width="17" height="15" rx="2.4" {...shared} />
          <path d="M15 4.5v15" {...shared} />
        </>
      )}
      {name === "spark" && (
        <>
